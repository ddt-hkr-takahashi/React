...
@Mapper
public interface BranchesListMapper
		extends CommonCountMapper, CommonDeleteMapper, CommonInsertMapper<BranchesList>, CommonUpdateMapper {
	@Generated(value = "org.mybatis.generator.api.MyBatisGenerator", date = "2025-09-02T10:18:59.990486+09:00", comments = "Source Table: branches_list")
	...
	default int deleteByPrimaryKey(Integer branchCode_) {
		return delete(c -> c.where(branchCode, isEqualTo(branchCode_)));
	}

	@Generated(value = "org.mybatis.generator.api.MyBatisGenerator", date = "2025-09-02T10:18:59.990486+09:00", comments = "Source Table: branches_list")
	default int insert(BranchesList row) {
		return MyBatis3Utils.insert(this::insert, row, branchesList, c -> c.map(branchCode).toProperty("branchCode")
	...
	default int insertMultiple(Collection<BranchesList> records) {
		return MyBatis3Utils.insertMultiple(this::insertMultiple, records, branchesList,
				c -> c.map(branchCode).toProperty("branchCode")
						.map(prefCode).toProperty("prefCode")
						.map(storeCode).toProperty("storeCode")
						.map(branchName).toProperty("branchName"));
	}
	...
	default int insertSelective(BranchesList row) {
		return MyBatis3Utils.insert(this::insert, row, branchesList,
				c -> c.map(branchCode).toPropertyWhenPresent("branchCode", row::getBranchCode)
	...
	default Optional<BranchesList> selectByPrimaryKey(Integer branchCode_) {
		return selectOne(c -> c.where(branchCode, isEqualTo(branchCode_)));
	}
	...
	default int updateByPrimaryKey(BranchesList row) {
		return update(c -> c.set(prefCode).equalTo(row::getPrefCode)
				.set(storeCode).equalTo(row::getStoreCode)
				.set(branchName).equalTo(row::getBranchName)
	...
	default int updateByPrimaryKeySelective(BranchesList row) {
		return update(c -> c.set(prefCode).equalToWhenPresent(row::getPrefCode)
				.set(storeCode).equalToWhenPresent(row::getStoreCode)
				.set(branchName).equalToWhenPresent(row::getBranchName)
				.where(branchCode, isEqualTo(row::getBranchCode)));
	}
